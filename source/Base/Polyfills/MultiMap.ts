type MultiMapEntry<K, V> = [K, V[]];
type MultiMapIterable<K, V> = MultiMapEntry<K, V>[];
type MultiMapIterationHandler<K, V> = (values: V[], key: K, multiMap: MultiMap<K, V>) => void;
...
export default class MultiMap<K, V> {
  ...
  public get (key: K): V[] {
    ...
    return keyIndex > -1 ? this._valueIterables[keyIndex] : null;
  }
